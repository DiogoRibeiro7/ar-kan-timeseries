...
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, ARMemory]:
...
def grid_search_arima(
    series: np.ndarray, grid: Iterable[ARIMAConfig]
) -> tuple[ARIMAConfig, float, dict[str, Any]]:
    ...
    report: dict[str, Any] = {}
    best: tuple[ARIMAConfig | None, float] = (None, float("inf"))
    ...
    for cfg in grid:
        try:
            ...
            hist = list(train)
            for x_true in test:
                m = ARIMA(hist, order=(cfg.p, cfg.d, cfg.q)).fit(
...
def grid_search_arkan(
    ...
    lr: float = 3e-3,
    device: str = "cpu",
) -> tuple[ARKANConfig, float, dict[str, Any]]:
    ...
    if series.ndim != 1:
        raise ValueError("series must be 1D.")
    report: dict[str, Any] = {}
    best: tuple[ARKANConfig | None, float] = (None, float("inf"))
